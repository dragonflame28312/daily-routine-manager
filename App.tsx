...
import { ArrowLeft, ArrowRight, X, Calendar, CalendarDays } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black text-gray-200 flex flex-col items-center justify-center font-sans">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-800/10 via-black/50 to-black" />
      </div>

      {/* Main App Shell */}
      <div className="relative z-10 w-full max-w-[1800px] h-full flex flex-col md:justify-center p-4 md:p-8 lg:p-12 gap-6">
        ...
        <header className="flex flex-col gap-4 text-center md:text-left md:flex-row md:items-end justify-between max-w-7xl mx-auto w-full">
          <div>
            <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-cyan-400 mb-2 pb-2">
              Daily Routine
            </h1>
            <p className="text-gray-400 max-w-2xl text-sm md:text-base font-medium drop-shadow-md">
          ...
          <div className="flex flex-col items-center md:items-end gap-3">
            <div className="flex gap-2 flex-wrap justify-center md:justify-end">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2 rounded-full bg-teal-600/20 border border-teal-500/50 hover:bg-teal-600/40 text-teal-200 transition-all text-sm font-bold uppercase tracking-wider shadow-lg hover:shadow-teal-900/20"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2 rounded-full bg-fuchsia-600/20 border border-fuchsia-500/50 hover:bg-fuchsia-600/40 text-fuchsia-200 transition-all text-sm font-bold uppercase tracking-wider shadow-lg hover:shadow-fuchsia-900/20"
              >
                <Calendar size={16} />
                View Full Schedule
              </button>
            </div>
            <div className="px-4 py-1.5 rounded-full bg-gray-900/60 border border-gray-800 backdrop-blur-sm">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-center max-w-7xl mx-auto w-full">
          <div className="flex items-center gap-2 p-1.5 rounded-full bg-gray-900/80 border border-gray-800 backdrop-blur-md shadow-2xl overflow-x-auto max-w-full no-scrollbar">
            <span className="text-[10px] uppercase font-bold text-gray-500 ml-3 mr-1">Time</span>
            {(['all', 'morning', 'midday', 'evening', 'allDay'] as TimeFilterType[]).map((t) => (
              <button
                ...
                className={`
                  ...
                  ${timeFilter === t
                    ? 'bg-fuchsia-500/20 text-fuchsia-200 border border-fuchsia-500/50 shadow-[0_0_15px_rgba(236,72,153,0.3)]'
          ...
          <div className="flex items-center gap-2 p-1.5 rounded-full bg-gray-900/80 border border-gray-800 backdrop-blur-md shadow-2xl overflow-x-auto max-w-full no-scrollbar">
            <span className="text-[10px] uppercase font-bold text-gray-500 ml-3 mr-1">Type</span>
            {(['all', 'supplements', 'face', 'hair', 'hands'] as CategoryFilterType[]).map((c) => (
              <button
                ...
                className={`
                  px-4 py-1.5 rounded-full text-xs font-medium transition-all duration-200 whitespace-nowrap
                  ${categoryFilter === c
                    ? 'bg-teal-500/20 text-teal-200 border border-teal-500/50 shadow-[0_0_15px_rgba(13,148,136,0.3)]'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800'}
                `}
              >
        ...
        <div className="relative w-full flex-1 min-h-[450px] flex flex-col justify-center">
          <button
            onClick={() => scroll('left')}
            className="hidden md:flex absolute left-4 z-20 w-12 h-12 items-center justify-center rounded-full bg-gray-800/80 border border-gray-600 text-white hover:bg-fuchsia-600 hover:border-fuchsia-400 transition-all shadow-xl backdrop-blur-sm group"
          >
          ...
          <button
            ...
            className="hidden md:flex absolute right-4 z-20 w-12 h-12 items-center justify-center rounded-full bg-gray-800/80 border border-gray-600 text-white hover:bg-fuchsia-600 hover:border-fuchsia-400 transition-all shadow-xl backdrop-blur-sm group"
          >
              ...
              <AnimatePresence mode='popLayout'>
                {filteredItems.length > 0 ? (
                  ...
                  <motion.div
                    initial={{ opacity: 0 }} 
                    animate={{ opacity: 1 }}
                    className="w-full text-center text-gray-500 italic py-12"
                  >
                    ...
                    <table className="w-full text-sm text-left text-slate-300">
                      ...
                      <tbody className="divide-y divide-slate-700/50 bg-slate-900/50">
                        {OVERVIEW_DATA.schedule.map((row, idx) => (
                          ...
                          <td className="px-4 py-3 font-medium text-teal-300">{row.time}</td>
                        ...
                        <tbody className="divide-y divide-slate-700/50 bg-slate-900/50">
                          {OVERVIEW_DATA.totals.map((row, idx) => (
                            <tr key={idx} className="hover:bg-slate-800/30">
                              ...
                              <td className="px-4 py-3 text-teal-300">{row.total}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      <table className="w-full text-sm text-left text-slate-300">
                        ...
                        <tbody className="divide-y divide-slate-700/50 bg-slate-900/50">
                          {OVERVIEW_DATA.topicals.map((row, idx) => (
                            <tr key={idx} className="hover:bg-slate-800/30">
                              <td className="px-4 py-3 font-medium text-white">{row.item}</td>
                              <td className="px-4 py-3 text-teal-200">{row.amount}</td>
